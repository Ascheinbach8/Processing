...
void cone (float radius, float height, int sides, float tipPercent) {
  int i,ii;
  float []c = new float[sides];
  float []s = new float[sides];
  ...
  for (i = 0; i < sides; i++) {
    ii = (i+1) % sides;
    beginShape(TRIANGLES);
    vertex (c[ii] * radius*tipPercent, 0.0, s[ii] * radius*tipPercent);
    vertex (c[i] * radius*tipPercent, 0.0, s[i] * radius*tipPercent);
    vertex (0.0, 0.0, 0.0);
    endShape();
  }
  ...
  for (i = 0; i < sides; i++) {
   ...
   cylinder(radius*tipPercent,height, sides);
}
